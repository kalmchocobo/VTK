...
int vtkSignedImplicitModeller::ComputeSignedDistance(vtkDataSet * input, int cellNum, double x[3], double cp[3], double & sdist)
{
	// initialize outputs
	for(int i=0; i<3; i++)
		cp[i]=0.0;
	...
	else
    {
		...
		if ( bcoords[1] < 0.0 && bcoords[2] < 0.0 )
		{
			...
			if(dist2Point < dist2Line1 and dist2Point < dist2Line2)
			{
				// closest point p0
				closest = p0;
				sdist = dist2Point;
			...
			else if(dist2Line1 < dist2Point and dist2Line1 < dist2Line2)
			{
...
void SetOutputDistance(double distance, OT *outputValue, double capValue, double scaleFactor)
{
    ...
    {
		if (capValue && std::fabs(distance) > capValue) // clamping iff non-float type
		{
											...
											double maxDistance, OT *)
{
	...
	double pcoords[3];
	int outExt[6];
	double x[3], prevDistance2, distance;
	int subId;
	double closestPoint[3];
	double *weights = new double[input->GetMaxCellSize()];
	double maxDistance2;
	double *spacing, *origin;
	float fNormal_float[3], vNormal_float[3];
	double fNormal[3], vNormal[3], vec[3];
	...
	normalsGen->SetInput((vtkPolyData *)input);
		...
		for (k = outExt[4]; k <= outExt[5]; k++) 
		{
			...
			for (j = outExt[2]; j <= outExt[3]; j++)
			{
				...
				for (i = outExt[0]; i <= outExt[1]; i++) 
				{
					...
					if ( self->ComputeSignedDistance(input, cellNum, x, closestPoint, distance) != -1 and
